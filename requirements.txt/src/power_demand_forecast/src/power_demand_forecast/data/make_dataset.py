...
def make_synthetic_hourly(start="2024-01-01", days=120, seed=42) -> pd.DataFrame:
    ...
    df["demand_mw"] = base + hour_peak + weekend_drop + temp_effect + demand_noise
    df["demand_mw"] = df["demand_mw"].clip(lower=5000)
    return df
...
def main() -> None:
    ...
    df = make_synthetic_hourly()
    df.to_csv(out / "demand.csv", index=False)
    print(f"Saved: {out / 'demand.csv'}  rows={len(df)}")
